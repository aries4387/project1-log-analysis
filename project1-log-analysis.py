...
def most_popular_authors():
    query = '''
        ...
        from authors,articles,log where log.path = '/article/'
...
def days_requests_lead_to_errors():
    query = '''
        ...
        (count(*)*100.0) / requests.total_requests  as errors from log,
        (select time::date as date, count(*) as total_requests
        from log group by time::date) as requests
        where log.time::date= requests.date and log.status <> '200 OK'
        group by date,requests.total_requests
        having (log.count*100.0)/requests.total_requests > 1 ;
     '''
